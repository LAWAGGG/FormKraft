...
## 📌 Konvensi Pengembangan Penting

### Backend Laravel
- **Mass Assignment**: Semua model menggunakan `protected $guarded = []` untuk kemudahan mass-assignment atribut.
- **Validasi**: Validasi dilakukan secara manual menggunakan `Validator::make` langsung di controller (tidak menggunakan FormRequest). Jika gagal, gunakan helper `$this->validateError($val)` untuk mengembalikan respon 422 JSON terstandar.
- **Controller Helpers**: Base `Controller` menyediakan helper:
  - `validateError($validator)` (Respon 422)
  - `notFound()` (Respon 404)
  - `forbidden()` (Respon 403)

### Frontend React
- **Client API**: Konfigurasi Axios didefinisikan di `src/api/api.js`. Token otomatis dilampirkan dari `localStorage` ("token") sebagai header `Bearer`. Apabila mendapat respon 401 Unauthorized, client akan otomatis me-redirect responden ke halaman login `/`.
- **Desain & Gaya**: Tidak menggunakan framework CSS utility (seperti Tailwind). Semua styling terpusat di `src/App.css` dengan CSS Variables sebagai sistem desain terpadu.
